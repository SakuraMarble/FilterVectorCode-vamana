# 比较两种算法——“UNG”和“UNG+ACORN”——在相同参数下的性能表现，并输出为文件
import pandas as pd
import os
import re
import glob
...
def process_dataframe(df):
    ...
    如果有多条，则选择UNG_time(ms)最小的。
    """
    if df.empty:
        return pd.DataFrame()

    # 确保关键列是数值类型
    for col in ['Recall', 'UNG_time(ms)', 'QueryID']:
        df[col] = pd.to_numeric(df[col], errors='coerce')
    
    df.dropna(subset=['Recall', 'UNG_time(ms)', 'QueryID'], inplace=True)
    df['QueryID'] = df['QueryID'].astype(int)

    # 排序后，每个QueryID分组的第一行就是最优结果
    df_sorted = df.sort_values(by=['QueryID', 'Recall', 'UNG_time(ms)'], ascending=[True, False, True])
...
def main():
    ...
    for dataset in DATASETS_TO_PROCESS:
        ...
        # 以 UNG+ACORN 为基准，查找所有实验目录
        acorn_exp_dirs = glob.glob(os.path.join(BASE_RESULTS_DIR, 'UNG+ACORN', dataset, '*'))
        
        if not acorn_exp_dirs:
            print(f"[WARN] 在 {dataset} 数据集下未找到 'UNG+ACORN' 的实验目录。")
            continue

        print(f"[INFO] 找到 {len(acorn_exp_dirs)} 个 'UNG+ACORN' 实验, 开始逐一匹配...")
        ...
        for acorn_dir in acorn_exp_dirs:
            ...
            try:
                ...
                ung_final = ung_optimal[['QueryID', 'repeat', 'Lsearch', 'UNG_time(ms)', 'DistanceCalcs', 'Recall', 'EntryPoints']].rename(columns={
                    'repeat': 'repeat_U', 'Lsearch': 'Lsearch_U', 'UNG_time(ms)': 'Time(ms)_U', 'DistanceCalcs': 'DistCalcs_U', 'Recall': 'Recall_U'
                })

                # 处理 UNG+ACORN
                acorn_df = pd.read_csv(acorn_csv_path)
                acorn_optimal = process_dataframe(acorn_df)
                acorn_final = acorn_optimal[['QueryID', 'repeat', 'Lsearch', 'UNG_time(ms)', 'DistanceCalcs', 'Recall']].rename(columns={
                    'repeat': 'repeat_AU', 'Lsearch': 'Lsearch_AU', 'UNG_time(ms)': 'Time(ms)_AU', 'DistanceCalcs': 'DistCalcs_AU', 'Recall': 'Recall_AU'
                })
                ...
                final_columns_order = [
                    'QueryID', 'repeat_U', 'Lsearch_U', 'Time(ms)_U', 'DistCalcs_U', 'Recall_U',
                    'repeat_AU', 'Lsearch_AU', 'Time(ms)_AU', 'DistCalcs_AU', 'Recall_AU', 'EntryPoints'
                ]
