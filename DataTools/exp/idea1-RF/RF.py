...
MODEL_OUTPUT_BASE_DIR = os.path.join(BASE_RESULTS_DIR, "TrieMethodSelectorModel")
...
def build_random_forest_selector_enhanced(data, model_name, output_dir):
    ...
    try:
        ...
        data['Tree_Shape_Factor_V2'] = (data['trie_label_cardinality'] * data['trie_avg_branching_factor']) / (data['trie_avg_path_length'] + 1e-9)
        
        data['choose_method_T'] = (data['Time_ms_T'] < data['Time_ms_F']).astype(int)
        
        pre_computable_features = [
            'Avg_QuerySize', 'Avg_CandSize', 'trie_avg_path_length', 
            'trie_avg_branching_factor', 'trie_label_cardinality', 
            'trie_total_nodes', 'Tree_Shape_Factor_V2'
        ]
        ...
        # 定义模型的输入类型
        # [None, len(pre_computable_features)] 的含义是：
        #   - None:  模型可以接受任意数量的查询（批处理大小可变）。
        #   - len(pre_computable_features): 每个查询必须包含的特征数量。
        ...
        with open(report_filename, "w", encoding="utf-8") as f:
            ...
            f.write("可以使用 joblib.load() 函数加载该模型。\n")
